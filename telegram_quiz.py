...
class Update:
    ...
    status: QuizStatus = field(default=None)
    team: Team = field(default=None)
    answer: Answer = field(default=None)
...
class TelegramQuiz:
    def __init__(self, *, id: str,
        ...
        self.last_status_update_id: int = 0
    ...
    def _notify_status_update(self):
        self.last_status_update_id = self.quiz_db.increment_update_id_counter()
    ...
    def get_updates(self, update_id_greater_than: int) -> List[Update]:
        updates: List[Update] = []

        # Disallow quiz state changes & database updates until we read everything.
        with self.lock, self.quiz_db.lock:
            if self.last_status_update_id > update_id_greater_than:
                updates.append(Update(
                    update_id=self.last_status_update_id,
